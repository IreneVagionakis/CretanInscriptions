...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
  ...
  <xsl:template match="result/doc" mode="search-results">
    ...
    <li>
      <a href="{$result-url}">
        <xsl:value-of select="concat(str[@name='file_name'], '. ', arr[@name='document_title']/str)" />
      </a>
    </li>
  </xsl:template>
    ...
    <xsl:choose>
      ...
      <xsl:when test="doc">
        <ul id="search_results">
          <xsl:apply-templates mode="search-results" select="doc" ><xsl:sort select="str[@name='file_name']" order="ascending"/></xsl:apply-templates>
        </ul>
